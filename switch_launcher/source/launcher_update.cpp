...
#define CEMU_SWITCH_RELEASE_VERSION "1.1.1"
#endif

#ifndef CEMU_SWITCH_VERSION
#define CEMU_SWITCH_VERSION "unknown"
#endif

namespace
{
	constexpr char kLatestReleaseUrl[] =
		"https://api.github.com/repos/NaGaa95/Cemu-nx/releases/latest";
	constexpr char kReleaseAssetPrefix[] =
		"https://github.com/NaGaa95/Cemu-nx/releases/download/";
	...
	bool ParseRelease(const std::string& json, LauncherReleaseInfo& release, std::string& error)
	{
		...
		for (std::size_t index = 0; index < assets.size(); ++index)
		{
			...
			if (lowerName.size() < 4 || lowerName.substr(lowerName.size() - 4) != ".nro" ||
				...
				continue;
			if (bestAsset < 0 || lowerName == "cemu.nro")
			{
	...
	void SetCommonCurlOptions(CURL* curl)
	{
		...
		curl_easy_setopt(curl, CURLOPT_USERAGENT, "Cemu-nx-Updater/" CEMU_SWITCH_RELEASE_VERSION);
...
{
	return CEMU_SWITCH_RELEASE_VERSION;
}
